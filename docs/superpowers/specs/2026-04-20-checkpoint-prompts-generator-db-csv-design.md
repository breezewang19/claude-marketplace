...
`DXCP_WSMC` 格式：`案件类型-审查环节-审查点名称`，例如 `"行政-受(立)案情况-受案登记表规范性审查"`

三段式结构，第二段为审查环节名称，与 Excel B列对应。

匹配策略（按优先级）：
1. **精确匹配**：`DXCP_WSMC` 按 `-` 分隔取第二段，与 Excel B列完全一致
2. **包含匹配**：如果精确匹配失败，检查 B列值是否包含在 `DXCP_WSMC` 的第二段中（处理括号等差异，如 "受(立)案情况" vs "受立案情况"）
